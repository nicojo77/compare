#!/usr/bin/bash
#script:		sort_arin_year.sh
#author:		anon
#date:			06.05.2023
#modification:		06.05.2023
#purpose:		sort arin whois based on creation date
#usage:			
...
parse_whois_result(){
	...
	while read -r ip; do
		result=$(grep -i -m1 $1 $ip\_whois.txt | awk -F ' ' '{print $NF}' | awk -F '-' '{print $1}')
		...
		case $result in
			...
			*)
				# Secondary search
				result=$(grep -i -m1 $2 $ip\_whois.txt | awk -F ' ' '{print $NF}' | awk -F '-' '{print $1}')
...
primary_searched_item='regdate'
secondary_searched_item='update'
display_debug=false
