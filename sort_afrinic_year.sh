#!/usr/bin/bash
#script:		sort_afrinic_year.sh
#author:		anon
#date:			07.05.2023
#modification:		07.05.2023
#purpose:		sort afrinic whois based on creation date
#usage:			
	...
	while read -r ip; do
		...
		case $result in
			...
			*)
				...
				# Secondary search
				sec_result=$(grep -i -m1 $2 $ip\_whois.txt | awk '{print substr($NF,0,4);}')
				debug "sec. ip: $ip sec. result: $sec_result"
				case $sec_result in
					20[2-9][0-9])
...
primary_searched_item="creation date"
secondary_searched_item="updated date"
display_debug=true
